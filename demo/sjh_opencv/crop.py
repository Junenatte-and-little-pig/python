...
class BorderRm():
    ...
    def detectBorder(self, im, ori, width, length, side):
        ...
        for i, ix in enumerate(im):
            border[i] = (
                np.sum(ix) * 0.003921568627451,
                np.sum(ori[i]) * 0.003921568627451)
    ...
    def checkborder(self, border, width, length, side):
        ...
        for i in range(int(length * 0.375)):
            ...
            Nborder.append(
                (i, border[i][1][0] + border[length - i - 1][1][0], colorSum))
        ...
        if side == 'Y':
            ...
            B = 0.40
            C = 0.40
        if Bvalue < width * B or (
                Cvalue > width * C and Cvalue < width * (1 - C)):
            cw = 0
        return cw, length - cw
    ...
    def sobel(self, img):
        ...
        Xborder = self.detectBorder(absX, img, Yborder[1] - Yborder[0], imgw,
                                    'X')
